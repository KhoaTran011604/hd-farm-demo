...
export default async function AnimalQrPage({ params }: Props): Promise<React.JSX.Element> {
  ...
  const tSpecies = await getTranslations('animals.species');
  const tQr = await getTranslations('animals.qr');
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[70vh] gap-6 print:min-h-screen print:justify-start print:pt-8">
      <div className="text-center space-y-1 print:hidden">
        ...
        <p className="text-sm text-muted-foreground">{tQr('instruction')}</p>
      </div>
      ...
      <div className="text-center space-y-2">
        ...
        <div className="flex items-center justify-center gap-3 text-sm text-muted-foreground">
          <span>{tSpecies(animal.species)}</span>
          {animal.pen && <><span>·</span><span>{tQr('penShort')}: {animal.pen.name}</span></>}
        </div>
